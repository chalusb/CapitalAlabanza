...
(async () => {
  ...
  try {
    ...
    assert.equal(requests.length, 3, 'Prepare a three-verse cushion before playback');
    ...
    assert.equal(await page.locator('.audio-loading').count(), 0, 'A prepared verse must not show loading');
    await page.waitForFunction(() => window.audioInstances[0].src.startsWith('blob:'));
    ...
    await page.waitForFunction(() => document.getElementById('bible-status').classList.contains('audio-loading'));
